...
const bootSpriteImg =
    document.querySelector('.bootSpriteImg');

const bootSpriteFolders = [
    'adulte',
    'batman',
    'batte',
    'boxeur',
...
const bootSpriteStates = [
    'attack',
    'hit',
    'idle'
];

function randomizeBootSprite() {

    if (!bootSpriteImg) return;

    const randomFolder =
        bootSpriteFolders[
            Math.floor(
                Math.random() * bootSpriteFolders.length
            )
            ];
    ...
    const randomState =
        bootSpriteStates[
            Math.floor(
                Math.random() * bootSpriteStates.length
            )
            ];

    bootSpriteImg.src =
        `./sprites/${randomFolder}/${randomState}.svg`;
}

function runBootSequence(realLoadingPromise) {
    ...
    return new Promise(async (resolve) => {
        ...
        const interval = setInterval(() => {
            ...
            bootPercent.innerText =
                `${Math.floor(progress)}%`;

        }, 60);

        await realLoadingPromise;
        ...
        if (elapsed < minDuration) {
            ...
            await new Promise(r =>
                setTimeout(r, minDuration - elapsed)
            );
...
window.addEventListener('load', async () => {
    ...
    if (urlParams.has('combat')) {
        ...
        document.getElementById('simuView').style.display =
            'block';
